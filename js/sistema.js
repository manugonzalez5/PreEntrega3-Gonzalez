...
let porcentaje = solicitarDato(
    "numero",
    "Ingrese el porcentaje de aumento a aplicar o 0 para cancelar"
);
...
let busqueda = solicitarDato(
    "cadena",
    "Ingrese la palabra o frase, se buscará en nombre"
);
const result = productos.filter(
    (producto) =>
    producto.nombre.toLowerCase().indexOf(busqueda.toLowerCase()) != -1 ||
    (producto.rubro &&
        producto.rubro.toLowerCase().indexOf(busqueda.toLowerCase()) != -1)
);
mostrarTabla(result);
if (result.length == 0) {
...
console.log(
    `Se encontraron ${result.length} resultados para su búsqueda ${busqueda}`
);
}
...
producto.stock = solicitarDato(
    "numeroEntero",
    "Ingrese el stock disponible del producto",
    producto.stock
);
}
